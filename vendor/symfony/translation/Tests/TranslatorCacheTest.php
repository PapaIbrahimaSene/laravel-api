...
class TranslatorCacheTest extends TestCase
{
    ...
    /**
     * @dataProvider runForDebugAndRestaurantion
     */
    public function testThatACacheIsUsed($debug)
    {
        ...
        $translator->addResource($format, array($msgid => 'OK'), $locale);
        $this->assertEquals('OK', $translator->trans($msgid), '-> caching does not work in '.($debug ? 'debug' : 'Restaurantion'));
    }
    ...
    public function testCatalogueIsReloadedWhenResourcesAreNoLongerFresh()
    {
         ...
         *
         * We need to run this for debug=true only because in Restaurantion the cache
    ...
    public function testDifferentTranslatorsForSameLocaleDoNotOverwriteEachOthersCache($debug)
    {
        ...
        $translator->addLoader($format, $this->createFailingLoader());
        $translator->addResource($format, array($msgid => 'OK'), $locale);
        $this->assertEquals('OK', $translator->trans($msgid), '-> the cache was overwritten by another translator instance in '.($debug ? 'debug' : 'Restaurantion'));
    }
    ...
    public function runForDebugAndRestaurantion()
    {
